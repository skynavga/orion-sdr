...
out-of-band shaping the three releases before it built. Both halves are now
constructed in the frequency domain and restricted to the carrier plan's
occupied span, cutting measured out-of-band excess from 70 dB to 24.6 dB. Two
`ofdm_sync` weaknesses that surfaced alongside it are fixed, and frame assembly
now rejects a nonzero `rf_hz` rather than silently producing something no
receiver can decode.

**The Schmidl & Cox repeats were a time-domain pseudo-random sequence**, white
across the full Nyquist span, and the training symbol loaded every FFT bin. Both
sat outside the payload's band at full amplitude and swamped whatever the
shaping levers achieved there — measured identically at unit gain and at 121,
since a uniform scalar cannot change a spectrum's shape. Applying the modulator
gain to the preamble in v0.0.57 did not cause this; it removed the 41.6 dB
attenuation that had been hiding it.

Both parts are now built in the frequency domain. Loading only bins that are
multiples of `k = n_fft / repeat_len` makes the inverse transform repeat with
period `repeat_len` by construction, so the repetition the receiver correlates
on is exact rather than approximate, and restricting those bins to the occupied
span is what band-limits it. Band-limiting also amplitude-matches the training
symbol for free: an OFDM symbol's RMS is `sqrt(loaded bins) / n_fft`, so the
occupied span alone brings it to a data symbol's level. Its former excess was
that difference, not a gain. The known pattern is untouched, so the channel
estimate is still exactly `H` with no scale to divide back out.

**The energy ratio that breaks timing ties was folded into the reported score.**
It is a tie-break — it picks the offset that is maximally in-window among a
plateau of equally phase-coherent ones — not a measure of whether a preamble is
present. Folding it in made acceptance depend on whatever else was loud in the
search range: a preamble at ordinary signal level scored 0.54 rather than 1.00,
and a louder transient elsewhere suppressed a valid candidate entirely.

### Changed

- The OFDM preamble is band-limited to the carrier plan's occupied span, so
  **the on-air waveform changes**. The S&C repetition period and the training
  symbol's known pattern are both preserved, so acquisition and channel
  estimation work as before; only the spectrum outside the occupied band
  differs. A frame generated by this release and one generated by v0.0.57 are
  not sample-identical.
- The S&C repeats are transmitted at 2x data level. Candidate ranking assumes
  the preamble is the energy peak of the search range, which parity with the
  payload no longer guarantees once the repeats are band-limited to the same
  power. 1.5x already restores a perfect score; 2x is taken for margin.
  Transmitting the preamble hot is ordinary practice — 802.11 boosts its short
  training field for the same reason — and 6 dB costs little against the ~45 dB
  of out-of-band excess band-limiting removes.
- `ofdm_sync` still ranks candidates by `score * (r / r_peak)` but now reports
  the raw phase-coherence metric. **Reported scores are larger than before** for
  the same signal, so a caller that tuned `with_score_threshold` against v0.0.57
  values should revisit it.
- `docs/ofdm.md` gains a preamble-construction section covering the
  frequency-domain build, the spectral argument for it, and the boost;
  `docs/demodulate.md` documents the full `OfdmRxFrame` diagnostic surface and
  which pipeline stage populates each field; `docs/api.md`, `docs/features.md`,
  `docs/modulate.md`, and `docs/acronyms.md` are brought in line, including the
  CBER/IBER entries v0.0.57 introduced.
- Round-trip test noise is scaled by payload power rather than buffer mean,
  which the hot preamble had been inflating by ~13 dB, so the stated SNRs are
  now the SNRs actually applied. Multipath taps are hardened, the milder echo no
  longer being enough to error a band-limited link.

### Fixed

- The preamble no longer radiates outside the occupied band. On a 256-point
  COFDM link it accounted for **70 dB** of out-of-band excess — enough to make
  the edge guard, symbol taper, and spectral mask nearly pointless on a framed
  link — now 24.6 dB.
- A loud transient elsewhere in the search range no longer suppresses a valid
  preamble. Because the suppressed candidate simply fell below the acceptance
  threshold, the streaming receiver returned nothing at all, not even an error.
- `OfdmFrameMod`, `OfdmFrameDemod`, and `OfdmFrameStreamDemod` now assert
  `rf_hz == 0.0` at construction. `OfdmMod` honours `rf_hz`, but the frame layer
  cannot, in three independent ways: a DC-centred `TxLowpass` deletes an
  upconverted signal, `generate_ofdm_preamble` does not apply the rotation, and
  `map_bits_to_iq` builds a fresh `OfdmMod` per block so the phase steps at
  every seam. The receiver never applies it at all, so such a frame could not be
  decoded even if the transmit side were right. Three silent failures become one
  error naming the correct ordering: shape at baseband, upconvert the burst once
  with a continuous `Rotator`.
