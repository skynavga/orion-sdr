...
See `docs/adr/0001-dvb-t-pilot-bracket-cache-in-equalizer.md` for why this
...
- **Cold-acquisition phase verification/recovery.** Investigating this work
  ...
  `project_ota_capture_hardware` agent memory, not as a separate `.scratch/`
  issue.
...
- MCS (DVB-T's TPS-signalled constellation + HP/LP code rate) and waveform
  ...
  MCS changes have zero interaction with this cache. See `CONTEXT.md`.
