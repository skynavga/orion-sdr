...
use orion_sdr::demodulate::{QamDecider, QamDemod};
use orion_sdr::modulate::{QamMapper, QamMod};

fn qam_roundtrip_noiseless<const BITS: usize>(n_syms: usize) {
    let n_bits = n_syms * BITS;
    let bits_in: Vec<u8> = (0..n_bits)
        .map(|i| ((i / BITS + i % BITS) & 1) as u8)
        .collect();
    ...
    assert_eq!(
        bits_in,
        bits_out,
        "QAM-{} noiseless roundtrip failed",
        1 << BITS
    );
}

#[test]
fn roundtrip_qam16_noiseless() {
    qam_roundtrip_noiseless::<4>(256);
}

#[test]
fn roundtrip_qam64_noiseless() {
    qam_roundtrip_noiseless::<6>(256);
}

#[test]
fn roundtrip_qam256_noiseless() {
    qam_roundtrip_noiseless::<8>(256);
}
