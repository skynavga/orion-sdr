...
- `.claude/CONTEXT.md` and `.claude/CONTEXT-MAP.md` (this repo's convention; a plain repo-root `CONTEXT.md`/`CONTEXT-MAP.md` is the pack's out-of-the-box default elsewhere)
- `.claude/docs/adr/` and any `src/*/docs/adr/` directories
- `.claude/docs/agents/`: does this skill's prior output already exist?
- `.claude/.scratch/`: a sign that a local-markdown issue tracker convention is already in use
...
**Section C: Domain docs.** Default to **single-context** (one `.claude/CONTEXT.md` + `.claude/docs/adr/`, this repo's convention). This fits almost every repo; write it without asking.
