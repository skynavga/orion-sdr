...
// === Phase 3: sync unit tests =============================================

// --- Waterfall ---

/// `compute_waterfall` should place peak energy in the correct bin for a pure tone.
#[test]
fn waterfall_peak_bin_matches_tone_frequency() {
    ...
    let tone_idx = 3usize; // transmit tone 3
    let f_tone = base_hz + tone_idx as f32 * FT8_TONE_SPACING_HZ;

    // Synthesise one symbol of a pure complex tone at f_tone.
    ...
    // Find the bin with maximum energy for symbol 0.
...
/// Energy in the correct bin should be significantly higher than adjacent bins.
#[test]
fn waterfall_peak_bin_dominates_neighbours() {
    ...
    // Peak should be at least 10 natural-log units above its neighbours.
...
/// Time offset in compute_waterfall correctly shifts the symbol window.
#[test]
fn waterfall_time_offset_shifts_window() {
    ...
    // Build a buffer: first n samples are silence, next n samples are the tone.
    ...
    // With time_offset=0 (points at silence), peak should NOT be at tone_idx.
    let wf0 = compute_waterfall(&buf, fs, base_hz, FT8_TONE_SPACING_HZ, n, 1, 8, 0);
    let e_silence = wf0.get(0, tone_idx);

    // With time_offset=n (points at the tone), peak should be at tone_idx.
...
/// Costas score should be substantially higher at the correct (t, f) than at wrong locations.
#[test]
fn costas_score_peaks_at_correct_location() {
    ...
    // Synthesise an FT8 frame (all-zeros data) and build waterfall.
    ...
    // Score at a wrong frequency bin should be lower.
    let score_wrong_freq = costas_score(&wf, &costas, &sync_pos, 0, 4);
    // Score at wrong time should also be lower.
...
/// find_candidates should return the correct (t, f) as the top result.
#[test]
fn find_candidates_top_hit_at_correct_location() {
    ...
    // Waterfall with extra bins so the search space includes some wrong answers.
...
// --- LDPC decoder regression (early-exit and best-plain fixes) ---

/// LDPC decoder should return 0 errors immediately when initial hard decisions are valid,
/// without running any BP iterations that might diverge.
#[test]
fn ldpc_decode_soft_early_exit_on_valid_initial_hard() {
    ...
    // Build moderate-magnitude soft LLRs with correct signs (as the sync extractor produces).
    // These have magnitude ~5, which previously caused BP divergence.
...
/// LDPC decode_soft must return the best plain even when BP diverges on later iterations.
/// Regression test for the missing best_plain tracking.
#[test]
fn ldpc_decode_soft_returns_best_plain_on_divergence() {
    use crate::codec::ldpc::{ldpc_decode_soft, ldpc_count_errors, N};

    // Construct a tricky LLR where the initial hard decision has some errors
    // but where a single BP iteration would correct them, then later iterations
    // might diverge.  We approximate this with a mixed-polarity LLR that has
    // correct signs for all codeword bits but very low magnitude.
    // The real regression guard here is: if errors is non-zero at the end,
    // the returned plain must be the best (minimum-errors) state, not the last.

    // For a valid codeword, ±0.1 magnitude should still have 0 syndrome errors
    // after any BP because the initial hard-decision path already works.
    // We just verify the function returns 0 and a coherent plain.
    ...
    // Very weak LLRs: magnitude 0.1 — BP may not converge but initial HD should.
    let llr: [f32; N] = core::array::from_fn(|i| {
        let bit = (codeword[i / 8] >> (7 - (i % 8))) & 1;
    ...
    // Verify the plain output satisfies the syndrome.
