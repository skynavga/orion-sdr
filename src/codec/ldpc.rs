...
// Code parameters:
//   N = 174  codeword bits
//   K = 91   information bits  (77 payload + 14 CRC)
//   M = 83   parity-check rows
//
// Encoder:    generator matrix in bitpacked form (kFTX_LDPC_generator).
// Decoder:    sum-product (belief propagation) using Nm / Mn / Num_rows.
//
// All constant arrays are derived from ft8_lib (kgoba, MIT licence).
//
// ── Codeword layout ────────────────────────────────────────────────────────
// The code is systematic: the first K=91 bits of the codeword are the message
// (a91) verbatim; the remaining M=83 bits are parity.
//
// K=91 does not fall on a byte boundary.  K_BYTES=12 (ceil(91/8)), so:
//   • codeword bytes 0-10: fully message bits
//   • codeword byte 11 (col_idx = K_BYTES-1 = 11): top 3 bits are message
//     (from a91[11] bits 7-5), bottom 5 bits are the first 5 parity bits.
//   • codeword bytes 12-21: remaining parity bits
//
// The encoder starts writing parity at bit position K=91:
//   col_mask = 0x80 >> (K % 8) = 0x80 >> 3 = 0x10  (bit 4 of byte 11)
//   col_idx  = K_BYTES - 1 = 11
//
// Consequence for tests: comparing codeword[..K_BYTES] against a91 is only
// valid for bytes 0-10; byte 11 differs in its lower 5 bits.
//
// ── NM array row count ─────────────────────────────────────────────────────
// NM has exactly 83 rows (M=83).  The ft8_lib source lists them sequentially;
// when transcribing, verify the count — the array is easy to truncate by two
// rows (rows 81 and 82, 0-indexed) because they appear after a visual gap in
// the source near the end of the array.

pub const N: usize = 174;
pub const K: usize = 91;
pub const K_BYTES: usize = 12; // ceil(91/8)
pub const N_BYTES: usize = 22; // ceil(174/8)
pub const M: usize = 83;

// ---------------------------------------------------------------------------
// Parity generator matrix — 83 rows × 12 bytes, MSB first.
// Row i gives the K-bit mask to dot-product against the message to produce
// parity bit i.
// ---------------------------------------------------------------------------
const GENERATOR: [[u8; K_BYTES]; M] = [
...
// ---------------------------------------------------------------------------
// Parity-check matrix Nm: each row lists the 1-based codeword indices
// that participate in that parity check.  Rows have 6 or 7 entries;
...
// Number of entries per NM row (6 or 7).
const NUM_ROWS: [u8; M] = [
...
// Mn: each codeword bit's 3 parity-check indices (1-based).
const MN: [[u8; 3]; N] = [
...
/// Encode a 91-bit message (12 bytes, MSB first) into a 174-bit codeword (22 bytes).
///
/// The first 91 bits of `codeword` are copied from `message`; the remaining
/// 83 bits are the LDPC parity bits computed using the generator matrix.
pub fn ldpc_encode(message: &[u8; K_BYTES], codeword: &mut [u8; N_BYTES]) {
    // Copy message bits into codeword
    for j in 0..N_BYTES {
        codeword[j] = if j < K_BYTES { message[j] } else { 0 };
    }

    // Compute parity bits and pack them after the K bits
...
/// Count the number of unsatisfied parity checks for a hard-decision codeword.
/// Returns 0 if the codeword is valid.
pub fn ldpc_count_errors(hard: &[u8; N]) -> usize {
    let mut errors = 0usize;
    for m in 0..M {
        let mut x: u8 = 0;
        for i in 0..(NUM_ROWS[m] as usize) {
            x ^= hard[NM[m][i] as usize - 1];
        }
        if x != 0 {
            errors += 1;
        }
...
// ---------------------------------------------------------------------------
// Soft-decision decoder (sum-product / belief propagation)
// ---------------------------------------------------------------------------
...
/// Soft-decision LDPC decoder (sum-product algorithm).
///
/// `llr`      — 174 log-likelihood ratios, LLR = log(P(x=0)/P(x=1)).
///              Positive ⇒ bit is more likely 0.
/// `max_iter` — maximum belief-propagation iterations (20 is typical).
/// `plain`    — output: 174 hard-decision bits (0 or 1).
///
...
pub fn ldpc_decode_soft(llr: &[f32; N], max_iter: usize, plain: &mut [u8; N]) -> usize {
    ...
    for _iter in 0..max_iter {
        ...
        // Variable-node hard decision
        for i in 0..N {
            let mut l = llr[i];
            for ji in 0..3 {
                l += e[MN[i][ji] as usize - 1][i];
            }
            plain[i] = u8::from(l <= 0.0);
        }
...
mod tests {
    use super::*;
    use crate::codec::crc::{ft8_add_crc, ft8_extract_crc};
    ...
    fn ldpc_encode_syndrome_zero() {
        let payload = [0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC, 0xDE, 0xF0, 0x11, 0x00];
        let a91 = make_a91(&payload);
        let mut codeword = [0u8; N_BYTES];
        ldpc_encode(&a91, &mut codeword);

        // Unpack codeword bits for syndrome check
        let mut hard = [0u8; N];
        for i in 0..N {
    ...
    fn ldpc_hard_roundtrip() {
        let payload = [0xDE, 0xAD, 0xBE, 0xEF, 0xCA, 0xFE, 0x01, 0x23, 0x45, 0x60];
        let a91 = make_a91(&payload);
        let mut codeword = [0u8; N_BYTES];
        ldpc_encode(&a91, &mut codeword);

        // Convert codeword bytes → LLRs (hard channel: ±10.0)
        ...
        // Check that the first K bits of plain match a91
        for i in 0..K {
            let expected = (a91[i / 8] >> (7 - (i % 8))) & 1;
            assert_eq!(plain[i], expected, "Bit {i} mismatch after decode");
        }

        // Verify CRC is intact: extract from recovered a91, then recompute over the
        // 77-bit payload portion (bits 0-76) zero-padded to 82 bits.
        ...
        // Zero out the CRC area (bits 77-90) before recomputing
        let mut buf_for_crc = recovered_a91;
        buf_for_crc[9] &= 0xF8;
        buf_for_crc[10] = 0;
        buf_for_crc[11] = 0;
        let crc_recomputed = crate::codec::crc::ft8_crc14(&buf_for_crc, 82);
        assert_eq!(crc_in_message, crc_recomputed, "CRC mismatch in decoded message");
    }

    #[test]
    fn ldpc_encode_preserves_message_bits() {
        let payload = [0xAB, 0xCD, 0xEF, 0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0x00];
        let a91 = make_a91(&payload);
        let mut codeword = [0u8; N_BYTES];
        ldpc_encode(&a91, &mut codeword);
        // First K=91 bits of codeword must match a91's first 91 bits.
        // Bytes 0..10 are fully message bits; byte 11 has 3 message bits (MSBs) + 5 parity bits.
        assert_eq!(&codeword[..K_BYTES - 1], &a91[..K_BYTES - 1]);
        // Top 3 bits of byte 11 come from a91[11]; bottom 5 are parity.
        let msg_mask: u8 = 0xE0; // top 3 bits
