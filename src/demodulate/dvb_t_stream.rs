...
// `dvb_t_frame_demodulate`, mirroring `OfdmFrameStreamDemod`. A DVB-T signal is a
...
// the link and supplied at construction, exactly as the batch entry point takes
...
use super::dvb_t_frame::{DvbTRxError, DvbTRxFrame, dvb_t_frame_demodulate};
...
impl DvbTFrameStreamDemod {
    ...
    fn try_one_frame(&mut self) -> FrameStep {
        ...
        // Decode the frame from its acquired start (the batch entry re-locks the
        // CP at offset 0 of this slice, which is idempotent).
        match dvb_t_frame_demodulate(
            self.params,
            &self.buf[start..],
            self.n_symbols,
            self.payload_len,
